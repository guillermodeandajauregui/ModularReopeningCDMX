...
plot_epicurve <- function(list_epiDF, my_colour = "red"){
  ...
  new_df <-
    data.frame(t = large_df$t, 
               ...
               #sd_ipc   = ifelse(is.na(da_SD), da_mean/2, da_SD)
    ) %>% as.tbl()
...
diagnostico_plot_epicurve <- function(list_epiDF, my_colour = "red"){
  #takes a list of epi-df (results of simulations)
  #returns a ggplot curve with mean and shadow
...
summary_epicurve <- function(list_epiDF){
  ...
  list_epiDF %>% 
    ...
    lapply(FUN = function(i){
      ...
      #repeak 
      repeak <- max_ipc > (pull(.data = i, I_pc)[1] + 0.1) #add 0.1 of tolerance to repeak
